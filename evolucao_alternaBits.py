...
def rotinaEvolucao(arqEnt, arqEvol):
    ...
    result = alterBitIndiv(melhorInd)   ###### função alterna bits
...
def evolui(TIPO, POP, D, Li, Ui, extra, sel, elite, cxTipo, cxProb, mutProb, passos, melhorFitLista, mediaFitLista, divParLista, divCentroLista):
    ...
    fit = alterBitPop(matriz) ##### função alterna bits
...
def alterBitPop(matriz):
    ...
    for i in range(0, POP):
        fit.append(alterBitFitIndiv(matriz[i]))
    
    return fit

def alterBitIndiv(indiv):
...
def alterBitFitIndiv(indiv):

    D = len(indiv)
    fo = alterBitIndiv(indiv)
